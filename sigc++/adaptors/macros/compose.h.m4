...
define([COMPOSE1_OPERATOR],[dnl
  ...
  operator()(LOOP(T_arg%1 _A_a%1, $1))
    { return this->functor_.template SIGC_WORKAROUND_OPERATOR_PARENTHESES<typename sigc::deduce_result_type<LIST(T_getter, LOOP(T_arg%1,$1))>::type>
        (get_(LOOP(_A_a%1, $1)));
    }
...
define([COMPOSE2_OPERATOR],[dnl
  ...
  typename deduce_result_type<LOOP(T_arg%1, $1)>::type
  operator()(LOOP(T_arg%1 _A_a%1, $1))
    { return this->functor_.template SIGC_WORKAROUND_OPERATOR_PARENTHESES<typename sigc::deduce_result_type<LIST(T_getter1, LOOP(T_arg%1,$1))>::type,
